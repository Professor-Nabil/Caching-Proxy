...
You are required to build a CLI tool
that starts a caching proxy server,
...
if the user makes a request to `http://localhost:3000/products`,
...
that indicate whether the response is from the cache or the server.
...
If the same request is made again,
the caching proxy server should return the cached response
instead of forwarding the request to the server.
...
After building the above project,
you should have a good understanding of how caching works
and how you can build a caching proxy server to cache responses from other servers.
